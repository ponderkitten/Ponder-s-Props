...
public class PowerDrillRightCLickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((world.getBlockState(new BlockPos(x, y + 1, z))).getBlock() == Blocks.AIR && entity.isShiftKeyDown() == true
				...
				&& (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == PondersRoleplayModModItems.POWER_DRILL.get()) {
			if (Math.round(Mth.nextInt(RandomSource.create(), 0, 1)) == 1) {
				world.setBlock(new BlockPos(x, y + 1, z), PondersRoleplayModModBlocks.POWER_DRILL_BLOCK.get().defaultBlockState(), 3);
			} else {
				world.setBlock(new BlockPos(x, y + 1, z), PondersRoleplayModModBlocks.POWER_DRILL_BLOCK_STANDING.get().defaultBlockState(), 3);
			}
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(PondersRoleplayModModItems.POWER_DRILL.get());
